...
export class Balance {
    ...
    */
    async _erreur (err) { // PRIVATE
        ...
        if (this.ecoute) { // est-on en mode écoute
            setTimeout(async () => {
                if (this.ecoute) { // testé à nouveau, en 3s l'indicateur a pu changer
    ...
    */
    async debutEcoute () {
        ...
        this.port.open(err => {
            if (err) {
                this._erreur(err)
            } else { // une fois le port ouvert, on enclenche le pooling de demande de poids
    ...
    */
    _onData (data) {
        ...
        setTimeout(() => { // on a reçu un poids, si on est toujours à l'écoute, on fait une nouvelle demande après un court délai
            if (this.ecoute) { // si on est toujours à l'écoute (il s'est passé 0,5s), on fait la nouvelle demande
                this._demandePoids()
            }
        }, delaiDemandePoids)
    }
